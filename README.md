...
程式更新後：每個 MCP 行程帶著啟動時的程式版本（`server/` 與協定檔的修改時間）。版本較新的 session 連上時，舊 hub 會通知其他 relay、讓出 port，由新版接手——常駐的舊行程（例如 Hermes gateway 帶起的 MCP）不會再擋住更新。`GET /status` 的 `build` 可看目前 hub 的版本。
（這個機制本身要新版才有：在它之前啟動、仍在跑的舊行程，需要手動結束一次。）
